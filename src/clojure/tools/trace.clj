...
(defn ^{:skip-wiki true} trace-compose-exception 
  ...
  (comment
    (let [klass  (class exception) 
          previous-msg (.Message exception)
          composed-msg(str previous-msg (if-not (.EndsWith previous-msg "\n") "\n") message (if-not (.EndsWith message "\n") "\n"))
          ctor (.GetConstructor klass (into-array [String]))
          new-exception ^Exception (cast klass (.Invoke ctor (into-array String [composed-msg])))
      ...
      new-exception))
  (let [previous-msg (.Message exception)
        composed-msg (str previous-msg (if-not (.EndsWith previous-msg "\n") "\n") message (if-not (.EndsWith message "\n") "\n"))]
    (Exception. composed-msg exception)))
